...
#
# Send reminders to speakers that have not registered, or that have
# a session in pending state that they have not confirmed.
#
...
if __name__ == "__main__":
	for conference in Conference.objects.filter(active=True, registrationtype__specialtype='spk'):
		# One transaction for each open conference that has registration
		# open. If registration isn't open then there is nowhere to
		# register, so don't even try. Also, only look for conferences
		# that actually have a registration type for speakers.
