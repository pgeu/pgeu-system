...
from .fyear import date_to_fy, fy_start_date, fy_end_date, format_fy_label
...
def year(request, year):
    ...
    try:
        ...
        if int(year) == date_to_fy(today_global()):
            ...
            year.save()
            messages.info(request, "Financial year {} did not exist in the system, but since it's the current financial year it has now been created.".format(year.label))
        else:
            return HttpResponse("Financial year {} does not exist in the system, and is not the current financial year.".format(format_fy_label(int(year))))
...
ORDER BY anum""".format(objstr),
        ...
        try:
            ...
            if prevyear and prevyear.isopen:
                messages.warning(request, 'Previous financial year (%s) is still open. Incoming balance will be incorrect!' % prevyear.label)
        except Year.DoesNotExist:
            pass
