# Verify balance on Qonto accounts
#
...
class Command(BaseCommand):
    ...
    class ScheduledJob:
        ...
        @classmethod
        def should_run(self):
            return InvoicePaymentMethod.objects.filter(active=True, classname='postgresqleu.util.payment.qonto.Qonto', config__verify_balances=True).exists()

    @transaction.atomic
    def handle(self, *args, **options):
        for method in InvoicePaymentMethod.objects.filter(active=True, classname='postgresqleu.util.payment.qonto.Qonto', config__verify_balances=True):
    ...
    def handle_method(self, method):
        ...
        if accounting_balance + pending != balance:
            send_simple_mail(settings.INVOICE_SENDER_EMAIL,
                             ...
                             'Qonto balance mismatch!',
                             """Qonto balance ({0}) for {1} does not match the accounting system ({2})!
