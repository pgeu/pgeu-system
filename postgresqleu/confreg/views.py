...
def admin_registration_dashboard(request, urlname):
	...
	# Discount codes
	curs.execute("SELECT code, maxuses, {0} FROM confreg_conferenceregistration r RIGHT JOIN confreg_discountcode dc ON dc.code=r.vouchercode WHERE dc.conference_id={1} AND r.conference_id=17 GROUP BY dc.id ORDER BY code".format(statusstr, conference.id))
